...
const UserState = (props) => {
  ...
  const addUser = async (newUser) => {
    const response = await fetch(`${URL}/api/user/adduser `, {
      ...
      body: JSON.stringify({ first_name: newUser.first_name, last_name: newUser.last_name, email: newUser.email, gender: newUser.gender, avatar: newUser.avatar, domain: newUser.domain, available: newUser.available }) 
    });
  ...
  const userUpdate = async (edata) => {
    ...
    const response = await fetch(`${URL}/api/user/update/${id}`, {
      ...
      body: JSON.stringify({ first_name: edata.efirst_name, last_name: edata.elast_name, gender: edata.egender, avatar: edata.eavatar, domain: edata.edomain, available: edata.eavailable }) 
    });
    const Json = await response.json();
    if (Json.success) {
      setQueries(Json);
  ...
  const getFilterData = async (data) => {
    ...
    if (jsonData.success) {
      ...
      toast.error('No data found with the specified filters.');
      setFilterData([]);
    }
  };
  ...
  const createTeam = (users) => {
    ...
    if (selectedUsers.some((user) => user.domain === domain)) {
      toast.success('User has the same domain as another user in the team.');
      return 1;
  ...
  const teamSubmit = async(data) => {
    const response = await fetch(`${URL}/api/team/addTeam`, {
      ...
      body: JSON.stringify({ teamName: data.teamName, description: data.description, member:data.member}) 
    });
    const userData = await response.json();
    if (userData.success) {
